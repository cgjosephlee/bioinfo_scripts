...
#
# source: https://github.com/10XGenomics/supernova/blob/master/tenkit/lib/python/tenkit/illumina_instrument.py
# ref: https://www.biostars.org/p/198143/
...
FCIDs = {
    ...
    "H[A-Z,0-9]{4}DSXX$": (["NovaSeq"], "S4 flow cell")
}
...
def sequencer_detection_message(fastq_files):
    ...
    if fails == len(seqrs):
        ...
        return iid, fcid, message, sequencers
...
def main():
    iid, fcid, message, sequencers = sequencer_detection_message(sys.argv[1:])
    print(f'IID:{iid}, FCID:{fcid}')
    print(message)
